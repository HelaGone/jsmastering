...
codes.ERR_UNKNOWN_FILE_EXTENSION = createError('ERR_UNKNOWN_FILE_EXTENSION', function (ext, path) {
...
function getPackageScopeConfig(resolved) {
  ...
  while (true) {
    ...
    var packageConfig = packageJsonReader.read((0, _url().fileURLToPath)(packageJSONUrl), {
      specifier: resolved
    });
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
  ...
  if (!main) {
    _process().emitWarning("No \"main\" or \"exports\" field defined in the package.json for ".concat(pkgPath, " resolving the main entry point \"").concat(urlPath.slice(pkgPath.length), "\", imported from ").concat(basePath, ".\nDefault \"index\" lookups for the main are deprecated for ES modules."), 'DeprecationWarning', 'DEP0151');
  } else if (_path().resolve(pkgPath, main) !== urlPath) {
    _process().emitWarning("Package ".concat(pkgPath, " has a \"main\" field set to \"").concat(main, "\", ") + "excluding the full filename and extension to the resolved file at \"".concat(urlPath.slice(pkgPath.length), "\", imported from ").concat(basePath, ".\n Automatic extension resolution of the \"main\" field is ") + 'deprecated for ES modules.', 'DeprecationWarning', 'DEP0151');
  }
}
...
function packageResolve(specifier, base, conditions) {
  ...
  do {
    ...
    var _packageConfig = packageJsonReader.read(packageJsonPath, {
      base: base,
      specifier: specifier
    });
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  ...
  var isRemote = protocol === 'http:' || protocol === 'https:';
  var resolved;
