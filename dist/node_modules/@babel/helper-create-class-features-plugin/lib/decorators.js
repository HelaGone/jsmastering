...
function transformClass(path, state, constantSuper, ignoreFunctionLength, className, propertyVisitor, version) {
  ...
  if (classDecorators) {
    ...
    var decoratorExpressions = classDecorators.map(function (el) {
      return el.expression;
    });
    var classDecsUsePrivateName = decoratorExpressions.some(usesPrivateField);
    var _handleDecoratorExpre = handleDecoratorExpressions(decoratorExpressions),
      hasSideEffects = _handleDecoratorExpre.hasSideEffects,
      decoratorsThis = _handleDecoratorExpre.decoratorsThis;
    var _generateDecorationLi = generateDecorationList(decoratorExpressions, decoratorsThis, version),
  ...
  if (hasElementDecorators) {
    ...
    try {
      for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
        ...
        if (hasDecorators) {
          var _decoratorExpressions = decorators.map(function (d) {
            return d.expression;
          });
          var _handleDecoratorExpre2 = handleDecoratorExpressions(_decoratorExpressions),
            _hasSideEffects = _handleDecoratorExpre2.hasSideEffects,
            usesFnContext = _handleDecoratorExpre2.usesFnContext,
            _decoratorsThis = _handleDecoratorExpre2.decoratorsThis;
          var _generateDecorationLi2 = generateDecorationList(_decoratorExpressions, _decoratorsThis, version),
          ...
          if (usesFnContext || _hasSideEffects && willExtractSomeElemDecs) {
  ...
  if (classDecorators) {
    ...
    if (statics.length > 0 || staticFieldInitializerExpressions.length > 0) {
      ...
      originalClassPath = _newPath.get("callee").get("body").get("body")[0].get("key");
    }
  }
  ...
  applyDecsBody.push(_core.types.expressionStatement(createLocalsAssignment(elementLocals, classLocals, elementDecorations, (_classDecorationsId = classDecorationsId) != null ? _classDecorationsId : _core.types.arrayExpression(classDecorations), _core.types.numericLiteral(classDecorationsFlag), needsInstancePrivateBrandCheck ? lastInstancePrivateName : null, _typeof(className) === "object" ? className : undefined, _core.types.cloneNode(superClass), state, version)));
  if (staticInitLocal) {
