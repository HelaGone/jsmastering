...
----------------------------------------------*/
	...
	//console.log( doubleNumbers( [2, 3, 6] ));
	...
	const goToTheMatrix = (arr)=>{
		const goToMatrix = arr.map(person=>{
			return ( person.age > 50 ) ? `${person.name} -> Goes to the Matrix` : `${person.name} -> Is under age`;
		});
		return goToMatrix;
	}
	...
	var capitalize = function(string){
	  // code code code!
	  const upperCase = Object.values(string).map(letter=>{
	...
	var swapCase = function(string){
	  ...
	  let words = string.split(' ');
		let count = 0;
	  const arr_words = words.map(word=>{
			count++;
	...
	var shiftLetters = function(string){
	  // code!
	  let str_obj = Object.values(string);
	  const mapped_str_obj = str_obj.map( str => {
	  	let codeAt = str.charCodeAt(0);
	  	return codeAt;
	  });

	  const shifted_str_obj = mapped_str_obj.map(str=>{
	...
	var countLetters = function(string){
		const object = string.split('').reduce((acumulador, current, index, arrSource)=>{
	...
	var isPresent = function(string, target) {
	  // GO GO GADGET CODE!
		return string.split('').reduce((prev, actual, index, matrix) => {
